...
class OdooAwait {
  ...
  constructor (opts) {
    ...
    // if no port is defined, then check to see if one is provided in the URL
    if (!this.port) {
      this.port = url.port;
    }
    // if not port is defined, then we will use the default according to http/https
    if (!this.port) {
      if (this.secure) {
        this.port = 443;
      } else {
   ...
   */
  async action(model, action, recordId){
